...
export const LibraryHeader = memo(
  ({ statusCounts, totalCount }: LibraryHeaderProps) => {
    ...
    return (
      <View style={styles.headerTopPadding}>
        <AccordionFilter
          ...
          onToggleLive={() => setFilters({ isLiveOnly: !filters.isLiveOnly })}
          statuses={statuses}
          selectedStatus={filters.status}
          onSelectStatus={(v) => setFilters({ status: v })}
          categories={categories}
          selectedCategory={filters.category}
          onSelectCategory={(v) => setFilters({ category: v })}
          periods={periods}
          selectedPeriod={filters.period}
          onSelectPeriod={(v) => setFilters({ period: v })}
        />
